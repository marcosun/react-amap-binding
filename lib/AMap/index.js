...
class AMap extends React.PureComponent {
  ...
  render() {
    ...
    // Pass map object instantiated by AMap.Map to all direct decendents
    // TODO: Refactor code structure: Handle multiple arrays of elements
    const childrenElement = () => {
      if (React.isValidElement(children[0])) { // Elements
        ...
        return children.map((child) => {
          return child instanceof Array ?
            child.map((subChild) => {
              return React.cloneElement(subChild, {map: map});
            }) :
            React.cloneElement(child, {map: map});
        });
      }
    };
